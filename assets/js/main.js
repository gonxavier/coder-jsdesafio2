...
if (despedida !="") {
    ...
    let cotiFinal = document.getElementById("cotizacion");

    cotiFinal.innerHTML = 
    `
    <h2>SU COTIZACION</h2>
    <p><strong>Vehiculo:</strong> ${consultaFinal.marca} ${consultaFinal.modelo} - (${consultaFinal.anio})</p>
    <p><strong>Cobertura:</strong> ${consultaFinal.producto}</p>
    <p><strong>Suma Asegurada:</strong> $ ${consultaFinal.sumaAsegurada}</p>
    <p class="costo"><strong>Costo Mensual:</strong> $ ${consultaFinal.valor}</p>
    `

    document.body.section.append(cotiFinal);
}
